class Battle
  def pbGainExpOne(idxParty, defeatedBattler, numPartic, expShare, expAll, showMessages = true)
    ...
    exp = (exp * 0.7).floor if trainerBattle?
    ...
    level_cap = pbLevelCap
    maximum_total_exp = growth_rate.minimum_exp_for_level(level_cap)
    exp_cap = maximum_total_exp - pkmn.exp
    exp_cap = 0 if exp_cap < 0

    expFinal = growth_rate.add_exp(pkmn.exp, [exp + exp_cap / 10, exp_cap].min) # add exp_cap / 10
    expGained = expFinal - pkmn.exp
    @leftover_exp = 0 if @leftover_exp.nil?
    @leftover_exp += exp - expGained

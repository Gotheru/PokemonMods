module Compiler
  ...
  def compile_types
    ...
    type_names = []
    pat = "{,modded_}types{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    # Read from PBS file
    paths.each do |path|
  ...
  def compile_abilities
    ...
    ability_hash         = nil
    pat = "{,modded_}abilities{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    paths.each do |path|
      compile_pbs_file_message_start(path)
      pbCompilerEachPreppedLine(path) { |line, line_no|
        if line[/^\s*\[\s*(.+)\s*\]\s*$/]   # New section [ability_id]
          ...
          if GameData::Ability.exists?(ability_id)
            ...
            # Will get overridden when registering.
          end
  ...
  def compile_moves
    ...
    move_descriptions = []
    move_hash         = nil
    pat = "{,modded_}moves{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
  ...
  def compile_items
    ...
    pat = "{,modded_}items{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    idx = 0
    paths.each do |path|
      compile_pbs_file_message_start(path)
  ...
  def compile_pokemon
    ...
    # Read from PBS file
    pat = "{,modded_}pokemon{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    paths.each do |path|
      compile_pbs_file_message_start(path)
      File.open(path, "rb") { |f|
  ...
  def compile_pokemon_forms
    ...
    used_forms = {}
    # Read from PBS file
    pat = "{,modded_}pokemon_forms{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    paths.each do |path|
      File.open(path, "rb") { |f|
        FileLineData.file = path   # For error reporting
  ...
  def compile_pokemon_metrics
    ...
    pat = "{,modded_}pokemon_metrics{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    anyProcessed = false
    # Read from PBS file
    paths.each do |path|
      next unless safeExists?(path)
      anyProcessed = true
      compile_pbs_file_message_start(path)
      File.open(path, "rb") { |f|
        FileLineData.file = path   # For error reporting
  ...
  def compile_encounters
    ...
    idx = 0
    pat = "{,modded_}encounters{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    paths.each do |path|
      compile_pbs_file_message_start(path)
      pbCompilerEachPreppedLine(path) { |line, line_no|
  ...
  def compile_trainer_types
    ...
    pat = "{,modded_}trainer_types{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    paths.each do |path|
      compile_pbs_file_message_start(path)
      pbCompilerEachPreppedLine(path) { |line, line_no|
  ...
  def compile_trainers
    ...
    idx = 0
    pat = "{,modded_}trainers{,_[0-9][0-9]*}.txt"
    paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
    paths.each do |path|
      compile_pbs_file_message_start(path)
      pbCompilerEachPreppedLine(path) { |line, line_no|
        ...
        if line[/^\s*\[\s*(.+)\s*\]\s*$/]
          ...
          when "EV"
            # We don't check EVs since we use insane amounts of EVs in HP for bosses xD
  ...
  def latest_change_time(path)
    times = [File.mtime(path).to_i, File.ctime(path).to_i]
    begin
      times << File.birthtime(path).to_i  # include if available
  ...
  CHECK = %w[abilities moves items pokemon pokemon_forms pokemon_metrics encounters trainer_types trainers]
  DATAFILES = %w[abilities berry_plants items map_connections map_metadata metadata moves phone player_metadata
                 regional_dexes ribbons shadow_pokemon species species_metrics town_map trainer_lists trainer_types
                 trainers types]
  COMPILATION_FILE = 'Data/LastCompilation.dat'
  class << self
    alias_method :__old_main, :main
    def main
      begin
        mustCompile = false
        latestDataTime = 0
        latestTextTime = 0
        CHECK.each do |type|
          pat = "{,modded_}#{type}{,_[0-9][0-9]*}.txt"
          paths = Dir.glob("PBS/#{pat}") + Dir.glob("Mods/**/#{pat}")
          paths.each do |path|
            next unless safeExists?(path)
            begin
              latestTextTime = [latestTextTime, latest_change_time(path)].max
            rescue SystemCallError
              # Ignored
            end
          end
        end
        DATAFILES.each do |type|
          path = "Data/#{type}.dat"
          if safeExists?(path)
            begin
              latestDataTime = [latestDataTime, latest_change_time(path)].max
            rescue SystemCallError
              mustCompile = true
            end
          else
        ...
        latestCompilation = (load_data(COMPILATION_FILE) rescue 0)
        mustCompile |= latestDataTime > latestCompilation
        mustCompile |= latestTextTime >= latestCompilation
        compile_all(mustCompile)
        save_data(Time.now.to_i, COMPILATION_FILE) if mustCompile
        __old_main
      rescue Exception
        e = $!
